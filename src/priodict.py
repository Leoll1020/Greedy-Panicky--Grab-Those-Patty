...
Rebuilds the heap if the number of deleted items gets large, to avoid memory leakage.'''
		...
		if len(heap) > 2 * len(self):
			...
			self.__heap.sort(key = self.__sort_by)  # builtin sort probably faster than O(n)-time heapify
		else:
			newPair = (val,key)
			insertionPoint = len(heap)
			heap.append(None)
			while insertionPoint > 0 and self.__sort_by(newPair) < self.__sort_by(heap[(insertionPoint-1)//2]):
